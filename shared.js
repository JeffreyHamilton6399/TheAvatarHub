...
(function () {
  ...
  const _run = function () {
    ...
    const NAV_SEL = [
      '.hub-card', '.card-cta', '.card-poster', 'a[href]:not([href^="#"]):not([href^="mailto"])'
    ].join(',');
    ...
    const CARD_HOVER_SEL = ['.ep-card', '.ep-thumb', '.merch-card', '.book-card', '.game-card'].join(',');
    const HOVER_SEL      = ['button', '[role="button"]', '.pill-btn', '.pill-tab', '.theme-opt', '.ctrl-btn', '.star-btn'].join(',');
    ...
    document.addEventListener('click', e => {
      ...
      // Settings open (gear button) — handled by panel open/close detection below
      if (t.closest('#settingsBtn'))      { /* handled below */return; }
      // Ambient toggle
    ...
    // Modal close buttons
    document.addEventListener('click', e => {
      if (e.target.closest('.modal-close, #installClose, [data-close]')) sfxClose();
    }, { passive: true });
    ...
    // Slider drag sounds — throttled to avoid spam
